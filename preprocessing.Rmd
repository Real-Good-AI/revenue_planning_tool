...
This is because the file were downloaded in 2024 prior to the new tier system. 
```{r}
...
## Dictionary
```{r}
data_dict <- read.csv("CORE/CORE-HRMN_dd.csv", show_col_types = FALSE)
head(data_dict)
#glimpse(data_dict)
#summarise(data_dict)
```
...
## Unified BMF Data
```{r}
unified_bmf <- read_csv("CORE/BMF_UNIFIED_V1.1.csv", show_col_types = FALSE)
head(unified_bmf)
#glimpse(unified_bmf)
...
You can skip this section if you want to get straight to cleaning the data.
...
unified_bmf <- read.csv("CORE/BMF_UNIFIED_V1.1.csv", show_col_types = FALSE)
...
CORE_1989 <- as.data.frame(read_csv("CORE/pz/CORE-1989-501C3-CHARITIES-PZ-HRMN.csv", show_col_types = FALSE))
CORE_2014 <- as.data.frame(read_csv("CORE/pz/CORE-2014-501C3-CHARITIES-PZ-HRMN.csv", show_col_types = FALSE))
CORE_2021 <- as.data.frame(read_csv("CORE/pz/CORE-2021-501C3-CHARITIES-PZ-HRMN.csv", show_col_types = FALSE))
CORE_2022 <- as.data.frame(read_csv("CORE/pz/CORE-2022-501C3-CHARITIES-PZ-HRMN.csv", show_col_types = FALSE))
...
# Which variables would we be dropping due to over 50% of records missing the corresponding data?
...
year_values <- seq(from = 2018, to = 2021, by = 1) # Download files from years in this list
download_CORE(tscope_values, fscope_values, year_values)
```
...
Columns corresponding to possible outcome variables
```{r}
# 990EZ: Part 2 line 25
# 990: Part 1 line 20 & Part 10 line 16
...
clean_BMF(vars_to_keep, thresh, saveFlag = TRUE) # if you set saveFlag to FALSE, the function instead returns a list of results that you would have to save into a variable i.e. cleanBMF.list <- clean_BMF(vars_to_keep, thresh, saveFlag = FALSE)
```
...
We need to do this because if they reported the same revenue each year, any GP / prediction stuff doesn't make sense for that org. Really you would just predict that same value that they reported each year. 
```{r}
table(df$DATA_COUNT)
quantile(df$DATA_COUNT, probs = seq(0,1,0.1))

df <- df |> filter(DATA_COUNT >= 5) # Drops about 600K records 
...
## Dropping Orgs with less than 5
```{r}
...
df.sub <- df |> group_by(EIN2) |> slice(1) |> ungroup()
```
...
quantile(df.sub$DATA_COUNT, probs = seq(0,1,0.1))

df.sub |> ggplot(mapping = aes(x = DATA_COUNT)) +
      geom_bar() +
...
print(plot_data_distributions(df = df, column = NTEE, tot = nrow(df), 
...
print(plot_data_distributions(df = df.sub, column = NTEE, tot = nrow(df.sub), title = "Proportion of total ORGANIZATIONS by subsector") + scale_x_discrete(labels = ntee_labs) +
                  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 0.9))
      )
print(plot_data_distributions(df = (df |> filter(DATA_COUNT < 5)), column = NTEE, 
                              title = "Proportion of total RECORDS by subsector, from Time Series with < 5 years")) + scale_x_discrete(labels = ntee_labs) +
                  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 0.9))
      
print(plot_data_distributions(df = df.sub |> filter(DATA_COUNT < 5), column = NTEE, tot = nrow(df.sub),
...
print(plot_data_distributions(df = df.sub |> mutate(SIZE.CAT = factor(SIZE.CAT)), column = SIZE.CAT, tot = nrow(df.sub),
...
print(plot_data_distributions(df = df.sub |> mutate(SIZE.CAT = factor(SIZE.CAT)) |> filter(DATA_COUNT < 5), column = SIZE.CAT, tot = nrow(df.sub),
...
print(plot_data_distributions(df = df.sub |> mutate(SIZE.CAT = factor(SIZE.CAT)) |> filter(DATA_COUNT < 4), column = SIZE.CAT, tot = nrow(df.sub),
                              title = "Proportion of total ORGANIZATIONS by size, from Time Series with < 5 years") + scale_x_discrete(labels = size_labs) +
                  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 0.9))
      )
```
...
print(plot_data_distributions(df = df.sub, column = DIVISION, tot = nrow(df.sub), 
...
print(plot_data_distributions(df = df.sub |> filter(DATA_COUNT < 5), column = DIVISION, tot = nrow(df.sub),
...
This assumes you've already run the outlier_detection.R script.
```{r}
df <- readRDS("PREPROCESSING/df.rds")
...
df <- df |> filter(DATA_COUNT >= 5) |>
      ...
      mutate(LOG_REV.mean = mean(LOG_REV.original))
      ungroup()
...
df <- df |> select(-F9_10_ASSET_TOT_EOY, -LOG_REV.mean)
...
# Add in the other information, like NTEE Code, SIZE, location, etc; note this is invariant to year (in theory); also recompute the LOG_REV mean after outlier removal; this is for the imputation step later
org.info <- df |> filter(!outlier) |>
      group_by(EIN2) |> 
      mutate(LOG_REV.mean = mean(LOG_REV.original)) |> 
      ungroup() |>
      select(-TOT_REV, -LOG_REV, -LOG_REV.original, -TAX_YEAR, -YEAR, -outlier, -IMPUTE_STATUS) |> 
...
### Step 2: Get predictions for missing years
```{r}
...
df <- df |> select(EIN2, TAX_YEAR, ORG_YEAR_FIRST, ORG_YEAR_LAST, LOG_REV, LOG_REV.original, outlier)

# Get first and last year per organization (prior to dropping outliers in case first or last year is outlier)
org_bounds <- df |>
      group_by(EIN2) |>
      summarize(FIRST_APPEARED = min(TAX_YEAR),
                LAST_APPEARED = max(TAX_YEAR),
                .groups = "drop")
saveRDS(org_bounds, "PREPROCESSING/orgs_first_last_appearance.rds")

# Get observed years per organization (after dropping outliers)
years.observed <- df |>
      filter(!outlier) |>
...
Refer to the file imputation_GP.R for the script that predicts missing values using GPs!

```{r}
# BONUS: How often is there a discrepancy between (ORG_YEAR_FIRST,ORG_YEAR_LAST) and (FIRST_APPEARED, LAST_APPEARED)
df <- df |> select(-TAX_YEAR, -LOG_REV, -LOG_REV.original) |> distinct()
df <- full_join(df, org_bounds, by = "EIN2")
df <- df |> mutate(DIFF_FIRST = (ORG_YEAR_FIRST != FIRST_APPEARED),
                   DIFF_LAST = (ORG_YEAR_LAST != LAST_APPEARED))

df <- df |> mutate(BMF_FIRST = (ORG_YEAR_FIRST < FIRST_APPEARED),
                   BMF_LAST = (ORG_YEAR_LAST > LAST_APPEARED))

df <- df |> mutate(ORG_YEAR_LAST.mod = if_else(ORG_YEAR_LAST %in% c(2022, 2023, 2024), 2021L, ORG_YEAR_LAST))

df <- df |> mutate(DIFF_LAST.mod = (ORG_YEAR_LAST.mod != LAST_APPEARED), 
                   BMF_LAST.mod = (ORG_YEAR_LAST.mod > LAST_APPEARED))
...
table(df$DIFF_LAST.mod)
table(df$BMF_LAST.mod)
```

### Step 3: Fill in the missing values in the skeleton

```{r}
# Step 3: Add missing data to the full dataset skeleton
missing.years.list <- readRDS("PREPROCESSING/years_to_predict.rds")

# First, any existing revenue values for outliers should be replaced with NA since they need to be imputed
df.full <- df.full |> mutate(TOT_REV = ifelse(IMPUTE_STATUS == "outlier", NA, TOT_REV),
                             LOG_REV = ifelse(IMPUTE_STATUS == "outlier", NA, LOG_REV),
                             LOG_REV.original = ifelse(IMPUTE_STATUS == "outlier", NA, LOG_REV.original))

# Next, let's put in the "imputed" values for orgs that didn't change, by filling in that value
orgs.no.change <- readRDS("PREPROCESSING/orgs_reported_same_after_outlier_removal.rds")
orgs.no.change <- missing.years.list[intersect(names(missing.years.list), orgs.no.change)] # the ones that need values imputed

# Change df.full to data.table for faster indexing
setDT(df.full)

# Get a data table with the values I will want to fill in for the orgs that reported no changes
df.sub <- df.full[data.table(EIN2 = names(orgs.no.change)), on = .(EIN2)]
df.sub <- df.sub |> drop_na(TOT_REV) |> select(-TAX_YEAR) |> distinct() |> select(EIN2, TOT_REV, LOG_REV.mean) # one row per org in orgs.no.change with their revenue values 

# Loop over EIN2s in orgs.no.change
start.time <- Sys.time() # ~ 5 minutes
for (ein in names(orgs.no.change)) {
      years.to.fill <- orgs.no.change[[ein]]
      
      # Get the replacement values from df.sub
      fill_values <- df.sub[EIN2 == ein, .(TOT_REV, LOG_REV.mean)]
      
      # If for some reason the EIN2 isn't found in df.sub, skip
      if (nrow(fill_values) == 0) {next}
      
      # Fill in the revenue values in df.full for matching EIN2 and TAX_YEAR
      df.full[(EIN2 == ein & TAX_YEAR %in% years.to.fill),
          `:=`(TOT_REV = fill_values$TOT_REV,
               LOG_REV.mean = fill_values$LOG_REV.mean)]
}
print(Sys.time() - start.time)

df.full <- df.full |> mutate(LOG_REV.original = ifelse(EIN2 %in% names(orgs.no.change), log(TOT_REV), LOG_REV.original),
       LOG_REV = ifelse(EIN2 %in% names(orgs.no.change), log(TOT_REV)-LOG_REV.mean, LOG_REV))

rm(fill_values, orgs.no.change, missing.years.list, df.sub, years.to.fill)
```

```{r}
# Next, we have to extract the predictions from the GP by putting the relevant info into a data frame
res.predictions <- readRDS("PREPROCESSING/imputation_results.rds")

start.time <- Sys.time() 
df.predictions <- do.call(rbind, lapply(names(res.predictions), function(id) {
  data.frame(EIN2 = id, TAX_YEAR = res.predictions[[id]]$TAX_YEAR, LOG_REV = res.predictions[[id]]$LOG_REV)
}))
print(Sys.time() - start.time)

# do the computation for LOG_REV.original and TOT_REV
df <- df.full |> select(EIN2, LOG_REV.mean) |> distinct() |> drop_na(LOG_REV.mean)
df.predictions <- left_join(df.predictions, df, by = "EIN2")
df.predictions <- df.predictions |>
      mutate(LOG_REV.original = LOG_REV + LOG_REV.mean) |>
      mutate(TOT_REV = exp(1) ** LOG_REV.original)

saveRDS(df.predictions, "PREPROCESSING/predictions_df.rds")
rm(res.predictions)

# Now we have to merge df.predictions with df.full
df.full <- df.full |> left_join(df.predictions, by = c("EIN2", "TAX_YEAR"), suffix = c("",".y"))
df.full <- df.full |> mutate(LOG_REV = coalesce(LOG_REV, LOG_REV.y),
                             LOG_REV.original = coalesce(LOG_REV.original, LOG_REV.original.y),
                             TOT_REV = coalesce(TOT_REV, TOT_REV.y))
df.full <- df.full |> select(-LOG_REV.y, -LOG_REV.original.y, -TOT_REV.y, -LOG_REV.mean.y)

rm(df.predictions)
# Check that we filled in the "inner" missing values 
# Get observed years per organization
# df.sub <- df.full |> drop_na(LOG_REV)
# years.observed <- df.sub |>
#       distinct(EIN2, TAX_YEAR) |>
#       group_by(EIN2) |>
#       summarize(YEARS_OBSERVED = list(TAX_YEAR), .groups = "drop") |>
#       deframe()
...
# # If the org has no missing years, no need to impute so remove
# missing.years.list <- Filter(function(x) length(x) > 0, missing.years.list)
# rm(df.sub, fill_values, full.range, missing.years.list, orgs.no.change, org_bounds, years.observed)

# Last, we add zeroes everywhere else!
df.full <- df.full |> mutate(TOT_REV = replace_na(TOT_REV, 1),
                             LOG_REV = replace_na(LOG_REV, 0),
                             LOG_REV.original = replace_na(LOG_REV.original, 0))

# For now, replace missing NTEE with "UNKNOWN" in case we want to search through them anyway
df.full <- df.full |> mutate(NTEE = replace_na(NTEE, "UNU"))

saveRDS(df.full, "PREPROCESSING/processed_mega_df.rds")
```

```{r}
df <- readRDS("PREPROCESSING/processed_mega_df.rds")

for (category in unique(df$NTEE)){
      df.sub <- df |> filter(NTEE==category)
      filename <- paste("PREPROCESSING/processed_mega_df_",category, ".rds", sep="")
      saveRDS(df.sub, filename)
      print(filename)
}

for (category in unique(df$NTEE)){
      filename <- paste("PREPROCESSING/processed_mega_df_",category, ".rds", sep="")
      df.sub <- readRDS(filename)
      print(filename)
      print(unique(df.sub$NTEE))
}
```

### Step 4: Checking some of the imputations
```{r}
df <- df.full |> filter(IMPUTE_STATUS != "imputed") |> select(EIN2, TAX_YEAR, TOT_REV, LOG_REV, LOG_REV.original, IMPUTE_STATUS)
df.outliers <- readRDS("PREPROCESSING/candidate_outliers_df.rds")
orgs.outlier.list <- unique(df.outliers$EIN2)

for (ein in sample(orgs.outlier.list, 10)){
      p <- df |> filter(EIN2 == ein) |>
            ggplot(aes(x = TAX_YEAR, y = LOG_REV)) +
            geom_line() +
...
### Step 5: Adding in organization names
```{r}
library(readr)
library(tidyverse, warn.conflicts = FALSE)
library(data.table)
...
for (category in unique(df$NTEE)){
      ...
      start.time <- Sys.time()
      filename <- paste("PREPROCESSING/processed_mega_df_",category, ".rds", sep="")
      df.cat <- readRDS(filename)
      print(Sys.time()-start.time)
}

df <- df |> select(EIN2, TAX_YEAR, TOT_REV, NTEE, IMPUTE_STATUS, ORG_NAME_CURRENT)
saveRDS(df, "PREPROCESSING/processed_mega_df_app.rds")
...
```{r}
source("SCRIPTS/clean_BMF.R")
res <- clean_BMF(vars_to_keep = c("EIN2", "ORG_NAME_CURRENT"), thresh = 10, saveFlag = FALSE)
res$na.count

org.names <- res$clean.data

df <- df |> left_join(org.names, by = "EIN2")

saveRDS(df, "PREPROCESSING/processed_mega_df.rds")
```