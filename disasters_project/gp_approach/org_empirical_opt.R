# I don't remember the exact time, but this file took me about 48 hours (nonstop) to run on my macbook
...
init_conds <- readRDS("gp_data/ORG_empirical_init_conds_50.rds")$centers
...
idx_list <- list("1" = 1:111640,
                 "2" = 111641:223280,
                 "3" = 223281:334920,
                 "4" = 334921:446560,
                 "5" = 446561:nrow(combos))

# idx_list <- list("1" = 1:10, # for testing; change %dopar% to %do% for testing but beware this will make it run sequential, not in parallel
#                  "2" = 100:110,
#                  "3" = 200:210,
#                  "4" = 1000:1010,
#                  "5" = 5000:5010)
...
start.time <- Sys.time() 
for (idx in names(idx_list)) {
      start.time.inner <- Sys.time()
      resBest <- foreach(i = idx_list[[idx]], .combine = 'rbind', .packages = c("dplyr", "fields", "mvtnorm"), .verbose = TRUE, .errorhandling = "remove") %dopar% {
            orgID <- combos$Var1[i]
      ...
      saveRDS(resBest, paste0("gp_data/ORG_empirical_best_", idx, ".rds"))
      timeDiff <- Sys.time() - start.time.inner
      print(paste0("ORG_empirical_best_", idx, ".rds"))
      print(timeDiff)
}
